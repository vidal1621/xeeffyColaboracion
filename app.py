...
from flask import Flask, render_template, request, redirect, flash, session, url_for, abort, jsonify, Response
...
def formulariocmf():
    ...
    tipo_empresa = request.form["empresa1"]
    nombre_empresa = request.form["nombre_empresa"]
    nemotecnico = request.form["Nemotecnico"]
    banco = request.form["banco"]
    banco = banco.replace(",]", "]")
    CMF = "SI"
    nivel1 = request.form["nivel1"]
    ...
    if validator == True:
        ...
        if test2 == []:
            cursor.execute(
                """insert into entidad (cod_entidad,desc_entidad,cod_tipo_entidad,cod_actividad_nivel1,cod_actividad_nivel2,cod_actividad_nivel3,cod_actividad_nivel4,cmf,comuna_id,nemotecnico) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);""",
                (
        ...
        if test2 != []:
            cursor.execute(
                """update entidad set desc_entidad=%s ,cod_tipo_entidad =%s, cod_actividad_nivel1=%s,cod_actividad_nivel2=%s,cod_actividad_nivel3=%s,cod_actividad_nivel4=%s,cmf=%s,comuna_id=%s, nemotecnico=%s where cod_entidad=%s""",
                (
...
def formularioEmpresaCMF():
    if session["username"] != 'unknown':
        cursor = cnx.cursor()
        cursor.execute("select cod_entidad, desc_entidad from entidad order by desc_entidad")
        entidad = cursor.fetchall()
        cursor.execute("select cod_clasificacion, desc_clasificacion from clasificacion order by desc_clasificacion")
        cl = cursor.fetchall()
        cursor.execute(
            "select sector,desc_actividad_entidad from actividad_entidad where cod_nivel=1 order by desc_actividad_entidad")
        lvl1 = cursor.fetchall()
        cursor.execute(
            "select sector, desc_actividad_entidad from actividad_entidad where cod_nivel=2 order by desc_actividad_entidad")
        lvl2 = cursor.fetchall()
        cursor.execute(
            "select sector, desc_actividad_entidad from actividad_entidad where cod_nivel=3 order by desc_actividad_entidad")
        lvl3 = cursor.fetchall()
        cursor.execute(
            "select sector, desc_actividad_entidad from actividad_entidad where cod_nivel=4 order by desc_actividad_entidad")
        ...
        return render_template("/formularioEmpresaCMF.html", entidad=entidad, region=region, comuna=comuna,
                               provincia=provincia, cl=cl,
...
def industria():
    if session["username"]:
        ...
        entidad = cursor.fetchall()
        cursor.execute(
            "select cod_clasificacion, desc_clasificacion from clasificacion order by desc_clasificacion")
        cl = cursor.fetchall()
        cursor.execute(
            "select sector,desc_actividad_entidad from actividad_entidad where cod_nivel=1 order by desc_actividad_entidad")
        lvl1 = cursor.fetchall()
        ...
        return render_template("/industria.html", entidad=entidad, cl=cl,
                               lvl1=lvl1, lvl2=lvl2, lvl3=lvl3, lvl4=lvl4)
    return redirect("login")


def get_balanceindustriaportadaactivos(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    # DEFINICION DE RATIOS
    RATIOS = {}
    RATIOS[
        'Corriente'] = "float(a.get(608) or 0) if (a.get(608,0)) else 0"
    RATIOS[
        'No corriente'] = "float(a.get(825) or 0)  if (a.get(825,0)) else 0"
    RATIOS[
        'Total'] = "float(a.get(808) or 0)  if (a.get(808,0)) else 0"

    # AÑOS
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from resumen_balanceindustria  where cod_actividad_nivel%s = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, nivel, cod_entidad, anio))
    ...
    cursor.execute(
        """SELECT DISTINCT
    cod_entidad,desc_entidad from entidad where cod_actividad_nivel%s ='%s' order by cod_entidad""" % (
            nivel, cod_entidad))
    entidades = cursor.fetchall()
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from resumen_balanceindustria  where cod_actividad_nivel%s = '%s' and anio in (%s) and mes in ('%s') order by cod_entidad"""
    cursor.execute(
        SQL_RESUMEN % (nivel, cod_entidad, "'%s', '%s'" % (anios[0], anios[1]), mes))
    ...
    for ratio in RATIOS:
        print(ratio)
        RESULTADO.append([ratio])

        for entidad in entidades:
            if len(RESULTADO) == 1: COLUMNAS1.append(entidad[1])
            print(entidad)
            b = {}
            for anio in anios:
                a = dict([(res[5], res[10]) for res in resumen if res[3] == entidad[0] and res[9] == anio])
                if len(RESULTADO) == 1: COLUMNAS2.append(CMon(anio, 0))
                RESULTADO[-1].append(CMon(eval(RATIOS[ratio]), 2))
                print(anio)
                b = a
    for linea in RESULTADO:
        if type(linea) == float:
            break
        promedio = {"actual": 0, "anterior": 0}
        actual = False
        print(linea[1:-1])
        for dato in linea[1:-1]:
            if actual:
                promedio["actual"] += float(CNumDb(dato))
            else:
                promedio["anterior"] += float(CNumDb(dato))
            actual = False if actual else True
        linea.append(CMon(promedio["actual"] / (len(COLUMNAS1) - 1)))
        linea.append(CMon(promedio["anterior"] / (len(COLUMNAS1) - 1)))
    COLUMNAS1.append("industria")
    COLUMNAS2.append(anios[0])
    COLUMNAS2.append(anios[1])
    return COLUMNAS1, COLUMNAS2, RESULTADO


def get_balanceindustriaportadapasivos(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    # DEFINICION DE RATIOS
    RATIOS = {}
    RATIOS[
        'Corriente'] = "float(a.get(668) or 0)  if (a.get(668,0)) else 0"
    RATIOS[
        'No corriente'] = "float(a.get(809) or 0)  if (a.get(809,0)) else 0"
    RATIOS[
        'Patrimonio'] = "float(a.get(681) or 0) if (a.get(681,0)) else 0"
    RATIOS[
        'Total'] = "float(a.get(754) or 0)  if (a.get(754,0)) else 0"
...
@app.route('/ResumenIndustria/<cod_entidad>/<mes>/<nivel>', methods=["GET", "POST"])
def ResumenIndustria(cod_entidad, mes, nivel):
    if session.get("username"):
        cursor = cnx.cursor()
        cursor.execute("select max(anio) from resumen_PortadaBalanceindustrias where cod_actividad_nivel%s = %s" % (
            nivel, cod_entidad))
        anio = cursor.fetchone()[0]
        parametros = {'cod_entidad': cod_entidad, 'nivel': nivel}
        cursor.execute("""SELECT
public.actividad_entidad.sector,
public.actividad_entidad.desc_actividad_entidad
FROM
public.actividad_entidad where actividad_entidad.sector = '%s'""" % (cod_entidad))
        r1 = cursor.fetchone()
        cursor.execute("""select cod_entidad,desc_entidad from entidad where cod_actividad_nivel%s =%s""" % (
            nivel, cod_entidad))
        rest = cursor.fetchall()
        cursor.execute("""select * from entidad where cod_actividad_nivel%s = '%s'""" % (nivel, cod_entidad))
        ...
        cursor.execute("""SELECT
                    ...
                    public.documento
                    INNER JOIN public.entidad ON public.documento.cod_entidad = public.entidad.cod_entidad
                    WHERE entidad.cod_actividad_nivel%s = '%s' and mes='%s'""" % (nivel, cod_entidad, mes))
        entidadC = cursor.fetchall()

        # BALANCE
        columnas, columnas2, data = get_balanceindustriaportadaactivos(cod_entidad, anio, mes, nivel)
        columnas5, columnas6, data5 = get_balanceindustriaportadapasivos(cod_entidad, anio, mes, nivel)
        # cursor.execute(
        #     """select * from resumen_PortadaBalanceindustrias where cod_actividad_nivel%s = '%s' and cod_cuenta='608' and mes='%s' ORDER BY cod_entidad""" % (
        #         nivel, cod_entidad,mes))
        ...
        #     corriente = "n/a"
        # cursor.execute(
        #     """select * from resumen_PortadaBalanceindustrias where cod_actividad_nivel%s = '%s' and cod_cuenta='825'  and mes='%s' ORDER BY cod_entidad""" % (
        #         nivel, cod_entidad,mes))
        ...
        #     Nocorriente = "n/a"
        # cursor.execute(
        #     """select * from resumen_PortadaBalanceindustrias where cod_actividad_nivel%s = '%s' and cod_cuenta='808'  and mes='%s' ORDER BY cod_entidad""" % (
        #         nivel,
        #         cod_entidad,mes))
        ...
        #     Total = "n/a"
        # cursor.execute(
        #     """select * from resumen_PortadaBalanceindustrias where cod_actividad_nivel%s = '%s' and cod_cuenta='668'  and mes='%s' ORDER BY cod_entidad""" % (
        #         nivel, cod_entidad,mes))
        # corrientepasivo = cursor.fetchall()
        # cursor.execute(
        #     """select * from resumen_PortadaBalanceindustrias where cod_actividad_nivel%s = '%s' and cod_cuenta='809' and mes='%s' ORDER BY cod_entidad""" % (
        #         nivel, cod_entidad,mes))
        ...
        # cursor.execute(
        #     """select * from resumen_PortadaBalanceindustrias where cod_actividad_nivel%s = '%s' and cod_cuenta='681' and mes='%s' ORDER BY cod_entidad""" % (
        #         nivel, cod_entidad,mes))
        ...
        #     patrimonio = "n/a"
        # cursor.execute(
        #     """select * from resumen_PortadaBalanceindustrias where cod_actividad_nivel%s = '%s' and cod_cuenta='754' and mes='%s' ORDER BY cod_entidad""" % (
        #         nivel, cod_entidad,mes))
        ...
        # RATIOS
        data3, columnas3 = get_portadaratiosddindustria(cod_entidad, anio, mes, nivel)
        # EERR
        data4, columnas4 = get_portadaeresultadoindustria(cod_entidad, anio, mes, nivel)
        return render_template("/ResumenIndustria.html", data3=data3, data=data, columnas=columnas, columnas5=columnas5,
                               columnas6=columnas6, data5=data5,
                               columnas3=columnas3, data4=list(data4), columnas4=columnas4, entidadC=entidadC,
                               retail=retail, r1=r1, nivel=nivel, cod_entidad=cod_entidad, columas2=columnas2)
    return redirect("/login")


def get_balanceindustria(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    # DEFINICION DE RATIOS
    RATIOS = {}
    RATIOS[
...
@app.route('/balanceindustria/<cod_entidad>/<anio>/<mes>/<nivel>', methods=["GET", "POST"])
def balanceindustria(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    parametros = {'cod_entidad': cod_entidad}
    cursor.execute(
        """select cod_entidad,desc_entidad from entidad where cod_actividad_nivel%s =%s""" % (nivel, cod_entidad))
    rest = cursor.fetchall()
    if not rest:
        abort(404)
    entidad = rest[1]
    columnas1, columnas2, resultado = get_balanceindustria(cod_entidad, anio, mes, nivel)
    return render_template("/balanceindustria.html", entidad=entidad, COLUMNAS2=columnas2, RESULTADO=resultado,
                           COLUMNAS1=columnas1, cod_entidad=cod_entidad, nivel=nivel)


# funcion extraccion ratios
def get_analisis_relativoindustria(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    aniom = int(anio) - 1
    RATIOS = {}
    RATIOS[
    ...
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from analisis_relativoindustria  where cod_actividad_nivel%s = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, nivel, cod_entidad, anio))
    anios = cursor.fetchone()
    ...
    entidades = cursor.fetchall()
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from analisis_relativoindustria  where cod_actividad_nivel%s = '%s' and anio in (%s) and mes in ('%s') order by cod_entidad"""
    cursor.execute(
        SQL_RESUMEN % (nivel, cod_entidad, "'%s', '%s'" % (anios[0], anios[1]), mes))
    resumen = cursor.fetchall()
    existan_datos = resumen != []
    ...
    for linea in RESULTADO:
        ...
        for dato in linea[1:-1]:
            if actual:
                promedio["actual"] += float(dato.replace(",", "."))
            else:
                promedio["anterior"] += float(dato.replace(",", "."))
            actual = False if actual else True
        linea.append(CMon(promedio["actual"] / (len(COLUMNAS1) - 1)))
        linea.append(CMon(promedio["anterior"] / (len(COLUMNAS1) - 1)))
    COLUMNAS1.append("industria")
...
@app.route('/analisisrelativoindustria/<cod_entidad>/<anio>/<mes>/<nivel>', methods=["GET", "POST"])
def analisisrelativoindustria(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    parametros = {'cod_entidad': cod_entidad}
    cursor.execute(
        """select cod_entidad,desc_entidad from entidad where cod_actividad_nivel%s =%s""" % (nivel, cod_entidad))
    ...
    if not rest:
        abort(404)
    entidad = rest[1]
    columnas1, columnas2, resultado = get_analisis_relativoindustria(cod_entidad, anio, mes, nivel)
    return render_template("/analisisrelativoindustria.html", entidad=entidad, COLUMNAS1=columnas1, COLUMNAS2=columnas2,
                           RESULTADO=resultado,
                           cod_entidad=cod_entidad, nivel=nivel)


# pagina estados de resutlados
@app.route('/eresultadoindustria/<cod_entidad>/<anio>/<mes>/<nivel>', methods=["GET", "POST"])
def eresultadoindustria(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    parametros = {'cod_entidad': cod_entidad}
    cursor.execute(
        """select cod_entidad,desc_entidad from entidad where cod_actividad_nivel%s =%s""" % (nivel, cod_entidad))
    ...
    columnas1, columnas2, resultado = get_eresultadoindustria(cod_entidad, anio, mes, nivel)
    return render_template("/eresultadoindustria.html", entidad=entidad, COLUMNAS1=columnas1, COLUMNAS2=columnas2,
                           RESULTADO=resultado,
                           cod_entidad=cod_entidad, nivel=nivel)


def get_eresultadoindustria(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    # DEFINICION DE RATIOS
    RATIOS = {}
    RATIOS[
    ...
    RATIOS[
        'Ganancia (pérdida)'] = "(float(a.get(479,1)))  if  a.get(479,0)  else 0"
    # AÑOS
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from resumen_eerrindustria  where cod_actividad_nivel%s = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, nivel, cod_entidad, anio))
    anios = cursor.fetchone()
    ...
    entidades = cursor.fetchall()
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from resumen_eerrindustria  where cod_actividad_nivel%s = '%s' and anio in (%s) and mes in ('%s') order by cod_entidad"""
    cursor.execute(
        SQL_RESUMEN % (nivel, cod_entidad, "'%s', '%s'" % (anios[0], anios[1]), mes))
    resumen = cursor.fetchall()
    existan_datos = resumen != []
    COLUMNAS = ["RATIOS"]
    COLUMNAS1 = ["ESTADO RESULTADO"]
    COLUMNAS2 = [""]
    RESULTADO = []
    for ratio in RATIOS:
        print(ratio)
...
@app.route('/industriaratiosdd/<cod_entidad>/<anio>/<mes>/<nivel>', methods=["GET", "POST"])
def industriaratiosdd(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    parametros = {'cod_entidad': cod_entidad}
    cursor.execute(
        """select cod_entidad,desc_entidad from entidad where cod_actividad_nivel%s =%s""" % (nivel, cod_entidad))
    ...
    columnas1, columnas2, resultado = get_ratiosindustria(cod_entidad, anio, mes, nivel)
    return render_template("/industriaratiosdd.html", entidad=entidad, empresa=empresa,
                           cod_entidad=cod_entidad, rest=rest, COLUMNAS1=columnas1, COLUMNAS2=columnas2,
                           RESULTADO=resultado, nivel=nivel)


# funcion extraccion ratios
def get_ratiosindustria(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    aniom = int(anio) - 1
    # DEFINICION DE RATIOS
    RATIOS = {}
    ...
    # AÑOS
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from resumen_ratiosindustria  where cod_actividad_nivel%s = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, nivel, cod_entidad, anio))
    ...
    entidades = cursor.fetchall()
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from resumen_ratiosindustria  where cod_actividad_nivel%s = '%s' and anio in (%s) and mes in ('%s') order by cod_entidad"""
    cursor.execute(
        SQL_RESUMEN % (nivel, cod_entidad, "'%s', '%s'" % (anios[0], anios[1]), mes))
    resumen = cursor.fetchall()
    existan_datos = resumen != []
...
def get_eresultado_pdf(cod_entidad, anio, mes):
    cursor = cnx.cursor()
    # DEFINICION DE RATIOS
    RATIOS = {}
    RATIOS[
    ...
    RATIOS[
        'Otras ganancias (pérdidas)'] = "(float(a.get(815,1)))  if  a.get(815,0)  else 0"
    RATIOS[
        'actividades operacionales'] = "(float(a.get(817,1)))  if  a.get(817,0)  else 0"
    RATIOS[
        'Ingresos financieros'] = "(float(a.get(558,1)))  if  a.get(558,0)  else 0"
    RATIOS[
        'Costos financieros'] = "(float(a.get(764,1)))  if  a.get(764,0)  else 0"
    RATIOS[
        'Participación en las ganancias (pérdidas)'] = "(float(a.get(824,1)))  if  a.get(824,0)  else 0"
    RATIOS[
        'Diferencias de cambio'] = "(float(a.get(849,1)))  if  a.get(849,0)  else 0"
    RATIOS[
        'Resultados por unidades de reajuste'] = "(float(a.get(516,1)))  if  a.get(516,0)  else 0"
    RATIOS[
        'importes en libros'] = "(float(a.get(1327,1)))  if  a.get(1327,0)  else 0"
    RATIOS[
        'Ganancia (pérdida), antes de impuestos'] = "(float(a.get(788,1)))  if  a.get(788,0)  else 0"
    RATIOS[
        'Gasto por impuestos a las ganancias'] = "(float(a.get(476,1)))  if  a.get(476,0)  else 0"
    RATIOS[
        'procedente de operaciones'] = "(float(a.get(774,1)))  if  a.get(774,0)  else 0"
    RATIOS[
        'Ganancia (pérdida)'] = "(float(a.get(479,1)))  if  a.get(479,0)  else 0"
    RATIOS[
        'propietarios de la controladora'] = "(float(a.get(704,1)))  if  a.get(704,0)  else 0"
    RATIOS[
        'no controladoras'] = "(float(a.get(868,1)))  if  a.get(868,0)  else 0"
    RATIOS[
        'Ganancia (pérdida)'] = "(float(a.get(479,1)))  if  a.get(479,0)  else 0"

    # AÑOS
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from resumen_eerr_pdf where cod_entidad = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
    ...
    * from entidad where cod_entidad ='%s'""" % (
    ...
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from resumen_eerr_pdf where cod_entidad = '%s' and anio in ('%s') and mes in ('%s')"""
    RESULTADO = {}
    cursor.execute(
        SQL_RESUMEN % (cod_entidad, anio, mes))
    resumen = cursor.fetchall()
    existan_datos = resumen != []
    COLUMNAS = ["E.RESULTADO"]
    b = dict()
...
def get_proyeccionEERR2(cod_entidad, anio, mes):
    ...
    * from entidad where cod_entidad ='%s'""" % (
    ...
    if dict(resultado).get('Otros gastos, por función'):
        RESULTADO['Otros gastos, por función'].append(dict(resultado).get('Otros gastos, por función',
                                                                          float(
                                                                              RESULTADO['Otros gastos, por función'][
                                                                                  -1]) /
                                                                          float(RESULTADO[
                                                                                    'Ingresos de Actividades ordinarias'][
                                                                                    -1])))
    ...
    if dict(resultado).get('Participación en las ganancias (pérdidas) de asociadas y negocios'):
        RESULTADO['Participación en las ganancias (pérdidas) de asociadas y negocios'].append(
            dict(resultado).get('Participación en las ganancias (pérdidas) de asociadas y negocios',
                                float(
                                    RESULTADO['Participación en las ganancias (pérdidas) de asociadas y negocios'][
                                        -1]) /
                                float(RESULTADO[
                                          'Ingresos de Actividades ordinarias'][
                                          -1])))
        RESULTADO['Ingresos financieros'].append(
            float(dict(resultado).get('Participación en las ganancias (pérdidas) de asociadas y negocios')) * float(
                RESULTADO['Ingresos de Actividades ordinarias'][-1]))

    elif RESULTADO['Participación en las ganancias (pérdidas) de asociadas y negocios'][-2]:
        RESULTADO['Participación en las ganancias (pérdidas) de asociadas y negocios'].append(
            dict(resultado).get('Participación en las ganancias (pérdidas) de asociadas y negocios', float(
                RESULTADO['Ingresos de Actividades ordinarias'][-1]) * float(
                RESULTADO['Participación en las ganancias (pérdidas) de asociadas y negocios'][-1])))
        RESULTADO['Participación en las ganancias (pérdidas) de asociadas y negocios'].append(
            dict(resultado).get('Participación en las ganancias (pérdidas) de asociadas y negocios', float(
                RESULTADO['Ingresos de Actividades ordinarias'][-1]) * float(
                RESULTADO['Participación en las ganancias (pérdidas) de asociadas y negocios'][-2])))
    else:
        RESULTADO['Participación en las ganancias (pérdidas) de asociadas y negocios'].append(0)
        RESULTADO['Participación en las ganancias (pérdidas) de asociadas y negocios'].append(0)
    ...
    if dict(resultado).get('Ganancias (pérdidas) que surgen de diferencias entre importes en libros'):
        RESULTADO['Ganancias (pérdidas) que surgen de diferencias entre importes en libros'].append(
            dict(resultado).get('Ganancias (pérdidas) que surgen de diferencias entre importes en libros',
                                float(
                                    RESULTADO[
                                ...
                                float(RESULTADO[
                                          'Ingresos de Actividades ordinarias'][
                                          -1])))
        RESULTADO['Ganancias (pérdidas) que surgen de diferencias entre importes en libros'].append(float(
            dict(resultado).get('Ganancias (pérdidas) que surgen de diferencias entre importes en libros')) * float(
            RESULTADO['Ingresos de Actividades ordinarias'][-1]))
        ...
        RESULTADO['Ganancias (pérdidas) que surgen de diferencias entre importes en libros'].append(
            dict(resultado).get('Ganancias (pérdidas) que surgen de diferencias entre importes en libros', float(
                RESULTADO['Ingresos de Actividades ordinarias'][-1]) * float(
                RESULTADO['Ganancias (pérdidas) que surgen de diferencias entre importes en libros'][-2])))
    else:
        RESULTADO['Ganancias (pérdidas) que surgen de diferencias entre importes en libros'].append(0)
        RESULTADO['Ganancias (pérdidas) que surgen de diferencias entre importes en libros'].append(0)
    RESULTADO['Ganancia (pérdida), antes de impuestos'].append(
...
def eresultado(cod_entidad, anio, mes):
    ...
    return render_template("/eresultado.html", entidad=entidad, data=list(data), columnas=columnas,
                           cod_entidad=cod_entidad)


@app.route('/verbalances', methods=["GET", "POST"])
def verbalances():
    if session["username"]:
        cur = cnx.cursor()
        cur.execute("""SELECT
                   ...
                   FROM
                   public.documento
                   INNER JOIN public.entidad ON public.documento.cod_entidad = public.entidad.cod_entidad LIMIT 10""")
        data = cur.fetchall()
        if request.args.get("anio"):
            ...
            FROM
            public.documento
            INNER JOIN public.entidad ON public.documento.cod_entidad = public.entidad.cod_entidad where documento.anio = %s """ % (
                anio))
            data1 = cur.fetchall()
            return render_template("/verbalances.html", dato=data1)
        return render_template("/verbalances.html", dato=data)
    return redirect("login")
...
def memo(cod_documento, cod_role):
    ...
    return render_template("/memo.html", d=data_final, columnas=columnas, entidad=entidad, desc_role=desc_role,
                           hoy=hoy, cod_documento=cod_documento,moneda=moneda,decimales=decimales)


@app.route('/topdf/<cod_documento>/<cod_role>', methods=["GET", "POST"])
def topdf(cod_documento, cod_role):
    config = pdfkit.configuration(wkhtmltopdf="C:\\Program Files\\wkhtmltopdf\\bin\\wkhtmltopdf.exe")
    pdfkit.from_url("http://192.168.1.168:5555/memo/" + cod_documento + "/" + cod_role, "balance.pdf",
                    configuration=config)
    resp = Response(open("balance.pdf", "rb").read())
    resp.headers['Content-Disposition'] = "inline; filename=balance.pdf"
...
def get_proyeccionRatios(cod_entidad, anio, mes):
    ...
    * from entidad where cod_entidad ='%s'""" % (
    ...
    cursor.execute(
        "select cuenta,valor from usuario_eerr where cod_entidad='%s' and usuario='%s'" % (cod_entidad, id_user))
    resultado = cursor.fetchall()
    RESULTADO['Dias de Caja'].append(dict(resultado).get('Dias de Caja', float(RESULTADO['Dias de Caja'][-1]) * 0.8 +
                                                         RESULTADO['Dias de Caja'][-2] * 0.2))
    ...
    RESULTADO['Dias de Inventarios'].append(
        dict(resultado).get('Dias de Inventarios', float(RESULTADO['Dias de Inventarios'][-1]) * 0.8 +
...
def get_ratiosdd_pdf(cod_entidad, anio, mes):
    cursor = cnx.cursor()
    aniom = int(anio) - 1
    # DEFINICION DE RATIOS
    RATIOS = {}
    ...
    RATIOS[
        'Tasa Interes Promedio'] = "float(a.get(764) or 0) / (((float(a.get(582) or 0) + float(a.get(791) or 0) + float(b.get(582) or 0)+ float(b.get(791) or 0)))/2)*100 if (a.get(764,0) and a.get(582,0)and a.get(791,0)  and b.get(582,0)and b.get(791,0)) else 0"
    # AÑOS
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from resumen_ratiosdd_pdf  where cod_entidad = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, cod_entidad, anio))
    ...
    * from entidad where cod_entidad ='%s'""" % (
    ...
    entidad = cursor.fetchone()[1]
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from resumen_ratiosdd_pdf  where cod_entidad = '%s' and anio in ('%s') and mes in ('%s')"""
    RESULTADO = {}
    cursor.execute(
        SQL_RESUMEN % (cod_entidad, anio, mes))
    resumen = cursor.fetchall()
...
def analisisRelativo(cod_entidad, anio):
    ...
    if not rest:
        abort(404)
    entidad = rest[1]
    data, columnas = get_analisisRelativo(cod_entidad, anio)
    return render_template("/analisisrelativo.html", entidad=entidad, data=list(data), columnas=columnas,
                           cod_entidad=cod_entidad)


def get_analisisRelativo(cod_entidad, anio):
    cursor = cnx.cursor()
    aniom = int(anio) - 1
    ...
    RATIOS[
        'Gastos por Impuestos a las Ganancias'] = "(float(a.get(476,0)) / float(a.get(540,0))*100) if (a.get(476,0) and a.get(540,0)) else 0"
    RATIOS[
        'Ganancias Procedentes Continuada'] = "(float(a.get(479) or 0) / float(a.get(540) or 0)*100) if (a.get(479,0) and a.get(540,0)) else 0"
    RATIOS[
        'Ganancias Porcentes Discontinuas'] = "(float(a.get(1328,0)) / float(a.get(540,0))*100) if (a.get(1328,0) and a.get(540,0)) else 0"
    RATIOS[
        'Ganancia (Perdida)'] = "(float(a.get(479,0)) / float(a.get(540,0))*100) if (a.get(479,0) and a.get(540,0)) else 0"
    ...
    # AÑOS
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from analisis_relativo  where cod_entidad = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, cod_entidad, anio))
    ...
    * from entidad where cod_entidad ='%s'""" % (
    ...
    entidad = cursor.fetchone()[1]
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from analisis_relativo  where cod_entidad = '%s' and anio in ('%s')"""
    RESULTADO = {}
    cursor.execute(
        SQL_RESUMEN % (cod_entidad, anio))
    resumen = cursor.fetchall()
    existan_datos = resumen != []
    COLUMNAS = ["ANALISIS RELATIVO"]
...
def get_nofresumen_pdf(cod_entidad, anio, mes):
    cursor = cnx.cursor()
    # DEFINICION DE RATIOS
    RATIOS = {}
    RATIOS[
    ...
    # AÑOS
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from nof_resumen  where cod_entidad = '%s' and anio >= '2016' and mes='12'"""
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, cod_entidad))
    anios = cursor.fetchone()
...
def get_portadaeresultado(cod_entidad, anio, mes):
    ...
    RATIOS[
        'GAV + Distribucion'] = "(float(a.get(1324,1))+float(a.get(618,1)),(float(a.get(1324,1))+float(a.get(618,1)))/float(a.get(540,1))*100  if  (a.get(1324,0))  else 0)"
    RATIOS[
...
def get_portadaratiosddindustria(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    aniom = int(anio) - 1
    # DEFINICION DE RATIOS
    RATIOS = {}
    ...
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from resumen_portadaratiosindustria  where cod_actividad_nivel%s = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, nivel, cod_entidad, anio))
    ...
    cursor.execute(
        """SELECT DISTINCT
    * from entidad where cod_actividad_nivel%s ='%s'""" % (
            nivel, cod_entidad))
    entidad = cursor.fetchone()[1]
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from resumen_portadaratiosindustria  where cod_actividad_nivel%s = '%s' and anio in ('%s') and mes in ('%s')"""
    RESULTADO = {}
    cursor.execute(
        SQL_RESUMEN % (nivel, cod_entidad, anio, mes))
    resumen = cursor.fetchall()
    existan_datos = resumen != []
    COLUMNAS = ["RATIOS"]
    b = dict()
...
def get_portadaeresultadoindustria(cod_entidad, anio, mes, nivel):
    cursor = cnx.cursor()
    # DEFINICION DE RATIOS
    RATIOS = {}
    RATIOS[
    ...
    # AÑOS
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from resumen_portadaeerrindustrias  where cod_actividad_nivel%s = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, nivel, cod_entidad, anio))
    ...
    * from entidad where cod_actividad_nivel%s ='%s'""" % (
    ...
    entidad = cursor.fetchone()[1]
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from resumen_portadaeerrindustrias  where cod_actividad_nivel%s = '%s' and anio in ('%s') and mes in ('%s')"""
    RESULTADO = {}
    cursor.execute(
        SQL_RESUMEN % (nivel, cod_entidad, anio, mes))
    resumen = cursor.fetchall()
...
def get_balance_pdf(cod_entidad, anio, mes):
    cursor = cnx.cursor()
    # DEFINICION DE RATIOS
    RATIOS = {}
    RATIOS[
    ...
    RATIOS[
        'Otros activos no financieros corrientes'] = "(float(a.get(835,1)))  if  a.get(835,0)  else 0"
    RATIOS[
        'Deudores comerciales y otras cuentas'] = "(float(a.get(775,1)))  if  a.get(775,0)  else 0"
    RATIOS[
        'Cuentas por cobrar, corrientes'] = "(float(a.get(787,1)))  if  a.get(787,0)  else 0"
    RATIOS[
        'Inventarios corrientes'] = "(float(a.get(695,1)))  if  a.get(695,0)  else 0"
    # RATIOS[
    #     'Activos biológicos corrientes'] = "(float(a.get(1290,1)))  if  a.get(1290,0)  else 0"
    RATIOS[
        'Activos por impuestos corrientes, corrientes'] = "(float(a.get(481,1)))  if  a.get(481,0)  else 0"
    RATIOS[
        'Total de activos corrientes distintos'] = "(float(a.get(797,1)))  if  a.get(797,0)  else 0"
    RATIOS[
        'Activos no corrientes o grupos'] = "(float(a.get(1291,1)))  if  a.get(1291,0)  else 0"
    RATIOS[
        'Activos corrientes totales'] = "(float(a.get(608,1)))  if  a.get(608,0)  else 0"
    RATIOS[
        'Otros activos financieros no corrientes'] = "(float(a.get(520,1)))  if  a.get(520,0)  else 0"
    ...
    RATIOS[
        'Cuentas por cobrar no corrientes'] = "(float(a.get(753,1)))  if  a.get(753,0)  else 0"
    # RATIOS[
    #     'Cuentas por cobrar, no corrientes'] = "(float(a.get(680,1)))  if  a.get(680,0)  else 0"
    # RATIOS[
    #     'Inversiones contabilizadas utilizando el método de la participación'] = "(float(a.get(713,1)))  if  a.get(713,0)  else 0"
    RATIOS[
        'Activos intangibles distintos de la plusvalía'] = "(float(a.get(748,1)))  if  a.get(748,0)  else 0"
    ...
    RATIOS[
        'Otros pasivos financieros corrientes'] = "(float(a.get(582,1)))  if  a.get(582,0)  else 0"
    RATIOS[
        'Cuentas por pagar comerciales'] = "(float(a.get(783,1)))  if  a.get(783,0)  else 0"
    # RATIOS[
    #     'Cuentas por pagar a entidades relacionadas, corrientes'] = "(float(a.get(461,1)))  if  a.get(461,0)  else 0"
    RATIOS[
        'Otras provisiones a corto plazo'] = "(float(a.get(659,1)))  if  a.get(659,0)  else 0"
    ...
    RATIOS[
        'Otros pasivos no financieros corrientes'] = "(float(a.get(621,1)))  if  a.get(621,0)  else 0"
    RATIOS[
        'Total de pasivos corrientes'] = "(float(a.get(745,1)))  if  a.get(745,0)  else 0"
    # RATIOS[
    #     'Pasivos corrientes totales'] = "(float(a.get(668,1)))  if  a.get(688,0)  else 0"
    RATIOS[
        'Otros pasivos financieros no corrientes'] = "(float(a.get(791,1)))  if  a.get(791,0)  else 0"
    ...
    RATIOS[
        'Otras reservas'] = "(float(a.get(789,1)))  if  a.get(789,0)  else 0"
    RATIOS[
        'Patrimonio atribuible'] = "(float(a.get(879,1)))  if  a.get(879,0)  else 0"
    RATIOS[
        'Participaciones no controladoras'] = "(float(a.get(620,1)))  if  a.get(620,0)  else 0"
    RATIOS[
        'Patrimonio total'] = "(float(a.get(681,1)))  if  a.get(681,0)  else 0"
    ...
    # AÑOS
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from resumen_balance_pdf  where cod_entidad = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, cod_entidad, anio))
    ...
    * from entidad where cod_entidad ='%s'""" % (
    ...
    entidad = cursor.fetchone()[1]
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from resumen_balance_pdf  where cod_entidad = '%s' and anio in ('%s') and mes in ('%s')"""
    RESULTADO = {}
    cursor.execute(
        SQL_RESUMEN % (cod_entidad, anio, mes))
    resumen = cursor.fetchall()
...
def get_analisisRelativo_pdf(cod_entidad, anio):
    cursor = cnx.cursor()
    aniom = int(anio) - 1
    ...
    # AÑOS
    SQL_ANIOS = """select coalesce(min(anio),%s), coalesce(max(anio),%s)  from analisis_relativo_pdf  where cod_entidad = '%s' and anio <= %s """
    anios = {}
    cursor.execute(
        SQL_ANIOS % (anio, anio, cod_entidad, anio))
    ...
    * from entidad where cod_entidad ='%s'""" % (
    ...
    entidad = cursor.fetchone()[1]
    # FIN ENTIDAD
    SQL_RESUMEN = """select distinct *  from analisis_relativo_pdf  where cod_entidad = '%s' and anio in ('%s')"""
    RESULTADO = {}
    cursor.execute(
        SQL_RESUMEN % (cod_entidad, anio))
    resumen = cursor.fetchall()
...
@app.route('/imprimetureporte/<cod_entidad>/<anio>/<mes>', methods=["GET", "POST"])
def imprimetureporte(cod_entidad, anio, mes):
    ...
    entidadC = cursor.fetchall()
    # NOF-FM
    data, columnas = get_nofresumen_pdf(cod_entidad, anio, mes)
    columna = columnas[1:]
    if columna == []:
        columna = "n/a"
    else:
    ...
    if Capital == "n/a" or Deudor == "n/a":
        ...
        porcientoCapital = (Capital / testructura) * 100
    # RATIOS
    data3, columnas3 = get_ratiosdd_pdf(cod_entidad, anio, mes)
    # EERR
    data4, columnas4 = get_eresultado_pdf(cod_entidad, anio, mes)
    # GRAFICO EERR
    data5, columnas5 = get_analisisRelativo_pdf(cod_entidad, anio)
    data6, columnas6 = get_balance_pdf(cod_entidad, anio, mes)
    hoy = datetime.now()
    hoy = hoy.strftime("%d-%m-%Y")
    return render_template("/imprimetureporte.html",entidad=entidad, data=list(data), columnas=columnas,
                           ...
                           columnas3=columnas3, data4=list(data4), columnas4=columnas4, entidadC=entidadC,
                           patrimonio=patrimonio, USD=USD, CLP=CLP, cod_entidad=cod_entidad, anio=anio, mes=mes,
                           data5=list(data5), columnas5=columnas5, hoy=hoy, data6=list(data6),columnas6=columnas6)


@app.route('/pdfreporte/<cod_entidad>/<anio>/<mes>', methods=["GET", "POST"])
def pdfreporte(cod_entidad,anio,mes):
    config = pdfkit.configuration(wkhtmltopdf="C:\\Program Files\\wkhtmltopdf\\bin\\wkhtmltopdf.exe")
    pdfkit.from_url("http://192.168.1.168:5555/imprimetureporte/" + cod_entidad + "/" + anio + "/"+ mes , "reporte.pdf",
                    configuration=config)
    resp = Response(open("reporte.pdf", "rb").read())
    resp.headers['Content-Disposition'] = "inline; filename=balance.pdf"
    resp.mimetype = 'application/pdf'
    return resp
...
def balance(cod_entidad, anio, mes):
    ...
    return render_template("/balance.html", entidad=entidad, data=list(data), columnas=columnas,
                           mes=mes, cod_entidad=cod_entidad)


def get_nofresumenentregable(cod_entidad, anio, mes):
    cursor = cnx.cursor()
    # DEFINICION DE RATIOS
    RATIOS = {}
    RATIOS[
...
@app.route('/informes/<cod_entidad>/<anio>/<mes>', methods=["GET", "POST"])
def informes(cod_entidad, anio, mes):
    cursor = cnx.cursor()
    parametros = {'cod_entidad': cod_entidad}
    cursor.execute("""select cod_entidad,desc_entidad from entidad where cod_entidad =%(cod_entidad)s""", parametros)
    rest = cursor.fetchone()
    if not rest:
        abort(404)
    entidad = rest[1]
    data3, columnas3 = get_ratiosdd(cod_entidad, anio, mes)
    data1, columnas1 = get_eresultado(cod_entidad, anio, mes)
    data, columnas = get_nofresumen(cod_entidad, anio, mes)
    columna = columnas[1:]
    if columna == []:
